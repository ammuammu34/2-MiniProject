...
public class EnquiryController {
	...
	public String getFilterEnquiries(@RequestParam String courses, @RequestParam String status,
			@RequestParam String classMode, Model model) {
	...
	public String edit(@RequestParam("id") Integer id, Model model) {
		...
		if (findById.isPresent()) {
			Student studentEnqEntity = findById.get();
			model.addAttribute("formobj", studentEnqEntity);
		}
		
		/*Student entity=enquiryservice.getStudent(id);
		EnquiryForm enquiryForm = new EnquiryForm();

		BeanUtils.copyProperties(entity, enquiryForm);

		initForm(model);
		model.addAttribute("enquiry", enquiryForm);*/
		return "add-enquiry";
	}

	/*
	 * @GetMapping("/error") public String handleError( HttpRequestHandler
	 * request,Model model) { Object status =
	 * model.getAttribute(RequestDispatcher.ERROR_STATUS_CODE);
	 * 
	 * if (status != null) { Integer statusCode =
	 * Integer.valueOf(status.toString());
	 * 
	 * if (statusCode == HttpStatus.NOT_FOUND.value()) { status = "error/404";
	 * 
	 * } else if (statusCode == HttpStatus.FORBIDDEN.value()) { status =
	 * "error/403";
	 * 
	 * } else if (statusCode == HttpStatus.INTERNAL_SERVER_ERROR.value()) { status =
	 * "error/500";
	 * 
	 * } }
	 * 
